...
public class ConsistencyCheck {
	 ...
	 *
	 * @param consistencyCheckDefinition Pid´s aller Konfigurationsbereiche, die in einer neuen Version geprüft werden sollen. Zu jedem Konfigurationsbereich ist
...
//						System.out.println("SystemObjekt: " + systemObject.getPid() + " Usage " + atgUsage.getUsage() + " ATG " + atgUsage.getAttributeGroup() + " Aspekt " + atgUsage.getAspect());
					...
					// Liste, die alle Attribute des Objekts enthält. Nur ATG´s und Attributlisten werden geprüft, in
					...
					if(systemObject instanceof SystemObjectType) {
						...
						if(configurationData != null && configurationData.isDefined()) {
							...
							for(int nr = 0; nr < arrayWithDefaultParameterDataSets.getLength(); nr++) {
								...
								// In dem Set sind nur Parametriende ATG´s enhalten. Ist die zu prüfende ATG ebenfalls parametrierend, muss der Rest ebenfalls
								// geprüft werden.
...
//					System.out.println("*****************************************************************************");
	 ...
	 * @param usedComponents Speichert alle Komponenten und deren übergordnete Objekte.
	 *
				...
				if(referenceAttributeType.getReferenceType() == ReferenceType.COMPOSITION) {
					...
					if(referencedObject instanceof ConfigurationObject) {
						if(configurationObjectAvailability((ConfigurationObject)referencedObject) == false) {
							// Das Objekt ist nicht mehr gültig, also tritt ein Interferenzefehler auf
